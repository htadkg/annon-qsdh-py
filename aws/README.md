...
# Configuration
1. This code pulls the docker image with tag:`latest` from [here](https://hub.docker.com/r/[INSERT YOUR USER NAME]/adkg/tags/). In order to replace the docker image, update `image_path` in `aws-config.json` with the appropriate image.
...
docker tag adkg:latest [INSERT YOUR USER NAME]/qsdh:latest # Replace with appropriate DockerHub location
docker push [INSERT YOUR USER NAME]/qsdh:latest # Replace with appropriate DockerHub location
